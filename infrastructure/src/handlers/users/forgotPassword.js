...
module.exports = (app) => {
  ...
  app.post('/v1/forgotPassword', auth.cors, async (req, res) => {
    ...
    try {
      ...
      const user = await postgres.findUser(dbClient, { email }, { projection: { auth: 1 } })
      if (!user) {
        logger.error(`User not found ${email}`)
        return api.sendErrorForbidden(res, api.createError('User not found', 'sign-in.auth-error'))
      }

      const resetToken = postgres.generateId(16)
...
const sendPasswordResetEmail = async (email, token) => {
  ...
  const context = {
    verificationLink: `${process.env.WEB_URL}/forgot-password/${token}`
  }
